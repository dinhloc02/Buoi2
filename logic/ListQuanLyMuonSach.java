...
public static void sapXep() {
		...
		for(int i=0; i<Main.QUANLYMUONSACH.length-1;++i) {
			if(Main.QUANLYMUONSACH[i]==null) continue;
			for(int j=i+1;j<Main.QUANLYMUONSACH.length;++j)
			{
				if(Main.QUANLYMUONSACH[j]== null ) continue;
			if(Main.QUANLYMUONSACH[i].getBanDoc().getHoTen().compareTo(Main.QUANLYMUONSACH[j].getBanDoc().getHoTen())>0) {
...
//				continue;
//			}
//			if(Main.QUANLYMUONSACH[i].getMuonSach().length>Main.QUANLYMUONSACH[j].getBanDoc().getHoTen().length()) {
//				
//			}
//		}
//	}
...
public static void searchBanDoc() {
	...
	for(int i=0;i<Main.QUANLYMUONSACH.length;++i) {
		if(Main.QUANLYMUONSACH[i]!=null && Main.QUANLYMUONSACH[i].getBanDoc().getHoTen().toUpperCase().contains(ten.toUpperCase()))
